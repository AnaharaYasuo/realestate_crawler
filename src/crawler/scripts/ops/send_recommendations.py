...
def send_recommendations():
    ...
    for eval_rec in candidates:
        ...
        # 売り出し価格の単位正規化 (1億円以上で万単位でない場合は10,000で割る)
        asking_price_man = asking_price // 10000 if asking_price > 500000 else asking_price
        ...
        # 判定条件1: アパート（投資用） - スコア 80.0 点以上に厳格化
        ...
        # 判定条件2: 居住用 (割安度 20.0%〜35.0% の信頼区間に厳選)
        elif pred_price > 0 and asking_price_man > 0:
            discount_pct = (pred_price - asking_price_man) / pred_price * 100.0
            if discount_pct >= 35.0 or discount_pct <= -35.0:
                logging.warning(f"Skipping alert (2-sigma error suspect): Property {prop.pageUrl} has extreme prediction gap of {discount_pct:.1f}% (Predicted: {pred_price:.0f}万円, Asking: {asking_price_man:.0f}万円)")
            elif discount_pct >= 20.0:
                is_recommend = True
                reason = f"【市場予測価格より {discount_pct:.1f}% 割安（超お宝物件！）】"
                priority_score = discount_pct
                
        if is_recommend:
            matched_candidates.append((priority_score, eval_rec, prop, reason))
    ...
    sent_count = 0
    for priority_score, eval_rec, prop, reason in top_candidates:
        asking_price = getattr(prop, "price", 0)
