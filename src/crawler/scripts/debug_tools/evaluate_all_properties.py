...
def _resolve_company_code(model_name: str) -> str:
...
def _estimate_single_property(item, ptype: str, company_code: str, api_base_url: str):
    """
    1物件のAPI推定とレコード更新を行う。
    成功時 (is_processed, is_evaluated, error_or_none) を返す。
    """
    ...
    with transaction.atomic():
        ...
        eval_record.first_stage_predicted_price = res_data["first_stage_predicted_price"]
        ...
        err = None
        if 100.0 <= asking_price_man < 30000.0 and pred2 > 0 and area >= 10.0:
            err = (asking_price_man - float(pred2)) / float(pred2)

        return (True, is_eval, err)


def _calculate_distribution_stats(errors: list):
    if not errors:
        return (0, 0.0, 0.0, 0.0, 0.0, "データ不足のため判定不能")
    ...
    return (n, mean_err, std_err, skewness, kurtosis, status_str)


def _evaluate_query_list(q_list, ptype: str, api_base_url: str):
    """特定の物件種別のクエリセット一覧を反復評価する"""
    total_p = 0
    total_e = 0
    errs = []
    for qs in q_list:
        company_code = _resolve_company_code(qs.model.__name__.lower())
        logging.info(f"  Estimating for table: {qs.model.__name__} (Total records: {qs.count()})")
        for item in qs:
            try:
                processed, evaluated, err = _estimate_single_property(item, ptype, company_code, api_base_url)
                if processed:
                    total_p += 1
                if evaluated:
                    total_e += 1
...
def main():
    ...
    for ptype, q_list in queries.items():
        ...
        p, e, errs = _evaluate_query_list(q_list, ptype, api_base_url)
        total_processed += p
        total_evaluated += e
        errors.extend(errs)
