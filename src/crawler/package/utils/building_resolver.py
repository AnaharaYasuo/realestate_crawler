...
def normalize_building_name(raw_name: str) -> str:
    ...
    name = re.sub(r"\d+号室?", "", name)
    name = re.sub(r"\d+階(?:部分)?", "", name)
...
def normalize_building_address(raw_address: str) -> str:
    ...
    # パターン1: ○丁目表記
    m_chome = re.search(r"^(.+?[都道府県]?.+?[市区町村].+?\d+丁目)", addr)
    if m_chome:
...
class BuildingResolver:
    ...
    def resolve_and_update(self, prop_data: Dict[str, Any]) -> Tuple[BuildingMaster, bool]:
        ...
        dev_tier = classify_developer_tier(dev_brand)
        contractor = prop_data.get("contractor_name")
        contractor_tier = classify_contractor_tier(contractor)
        ...
        if not bm:
            ...
            if updated:
                ...
                bm.save()
                logging.info(f"BuildingResolver: Enriched existing BuildingMaster [{norm_name}]")
