...
class ObjectStorageManager:
    ...
    def __init__(self):
        ...
        if self.backend == 'gcs' or (os.getenv('IS_CLOUD') and os.getenv('STORAGE_ENDPOINT') is None):
            ...
            self.s3_client = boto3.client('s3', endpoint_url=self.endpoint_url, aws_access_key_id=self.access_key, aws_secret_access_key=self.secret_key, config=Config(signature_version='s3v4'), region_name=os.getenv('AWS_DEFAULT_REGION', 'us-east-1'))
    ...
    def _ensure_bucket_exists(self):
        ...
        try:
            ...
            try:
                policy = {'Version': '2012-10-17', 'Statement': [{'Sid': 'PublicRead', 'Effect': 'Allow', 'Principal': '*', 'Action': ['s3:GetObject'], 'Resource': [f'arn:aws:s3:::{self.bucket_name}/*']}]}
    ...
    def upload_image_bytes(self, image_bytes: bytes, filename: str, content_type: str='image/jpeg') -> str:
        ...
        if self.is_gcs:
            try:
                ...
                raise
        try:
            self.s3_client.put_object(Bucket=self.bucket_name, Key=filename, Body=image_bytes, ContentType=content_type)
