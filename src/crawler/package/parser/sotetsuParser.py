...
from package.models.sotetsu import SotetsuMansion, SotetsuKodate, SotetsuTochi
from package.parser.baseParser import ParserBase
from package.utils import converter
from package.utils.selector_loader import SelectorLoader

class SotetsuParser(ParserBase):
    BASE_URL = 'https://www.sotetsu-re.co.jp'
    property_type = ""

    def __init__(self, params=None):
        self.selectors = SelectorLoader.load('sotetsu', self.property_type or 'mansion')
    ...
    async def getResponseBs(self, session, url, charset=None) -> BeautifulSoup:
        ...
        logging.info(f"SotetsuParser: Launching Playwright to render JS for URL: {url}")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logging.error(f"SotetsuParser Playwright failed: {e}")
    ...
    async def parseRootPage(self, response: BeautifulSoup):
        detail_links = set()
        for a in response.select('a[href*="/buy/view/"]'):
            href = a.get("href")
            if href:
    ...
    def _get_specs(self, response: BeautifulSoup) -> dict:
        specs = {}
        for row in response.select(".outlineTable .tr"):
    ...
    def _parsePropertyDetailPage(self, item, response: BeautifulSoup):
        ...
        # 物件名
        name_el = response.select_one(".bukkenname .name")
        if name_el:
            item.propertyName = name_el.get_text().strip()
        elif "物件名" in specs:
            item.propertyName = specs.get("物件名", "")
            
        # 価格
        price_el = response.select_one(".price .kakaku")
        if price_el:
            item.priceStr = re.sub(r'\s+', ' ', price_el.get_text()).strip()
            item.price = converter.parse_price(item.priceStr)
        elif "価格" in specs:
            item.priceStr = re.sub(r'\s+', ' ', specs.get("価格", "")).strip()
            item.price = converter.parse_price(item.priceStr)
...
class SotetsuMansionParser(SotetsuParser):
    property_type = 'mansion'

    def createEntity(self):
        return SotetsuMansion()
    ...
    def _parsePropertyDetailPage(self, item, response: BeautifulSoup):
        ...
        # 築年月
        item.chikunengetsuStr = specs.get("築年月", "") or specs.get("築年月（竣工予定）", "") or specs.get("完成時期", "")
...
class SotetsuKodateParser(SotetsuParser):
    property_type = 'kodate'

    def createEntity(self):
        return SotetsuKodate()
...
class SotetsuTochiParser(SotetsuParser):
    property_type = 'tochi'
