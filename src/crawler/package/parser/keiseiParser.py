...
class KeiseiParser(ParserBase):
    BASE_URL = 'https://www.keisei-land.co.jp'
    property_type = ""

    def __init__(self, params=None):
        self.selectors = SelectorLoader.load('keisei', self.property_type or 'mansion')
    ...
    async def getResponseBs(self, session, url, charset=None) -> BeautifulSoup:
        ...
        logging.info(f"KeiseiParser: Launching Playwright to render JS for URL: {url}")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logging.error(f"KeiseiParser Playwright failed: {e}")
        ...
        for a in response.select('a[href*="detail"]'):
            ...
            if href:
                ...
                if normalized not in detail_links:
                    ...
                    logging.info(f"[Keisei] Match detail link: {normalized}")
                    yield normalized
        ...
        for table in response.select("table"):
            for tr in table.select("tr"):
                ...
                for i in range(min(len(ths), len(tds))):
                    ...
                    val = re.sub(r'\s+', ' ', val)
                    specs[key] = val
        # 上部概要 ul.propertyInfoWidth01 の抽出
        for ul in response.select("ul.propertyInfoWidth01"):
    ...
    def _parsePropertyDetailPage(self, item, response: BeautifulSoup):
        ...
        # 価格
        price_el = response.select_one("#price, .price_box")
        price_val = ""
        if price_el:
            price_val = price_el.get_text().strip()
        else:
...
class KeiseiMansionParser(KeiseiParser):
...
class KeiseiKodateParser(KeiseiParser):
...
class KeiseiTochiParser(KeiseiParser):
    property_type = 'tochi'
