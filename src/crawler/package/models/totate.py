...
class TotateModel(PropertyBaseModel, TransportationMixin):
...
class TotateMansion(TotateModel):
    ...
    class Meta(TotateModel.Meta):
        db_table = "totate_mansion"

class TotateTochi(TotateModel):
...
class TotateKodate(TotateModel):
    tochiMensekiStr = models.TextField(blank=True)
    tochiMenseki = models.DecimalField(max_digits=10, decimal_places=3, null=True, blank=True)
