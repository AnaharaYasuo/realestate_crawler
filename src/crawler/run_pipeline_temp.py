...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(log_dir, "pipeline_temp.log"), encoding="utf-8")
    ]
)
...
def main():
    ...
    # クローラーが数千件あると数時間かかるため、テスト実行用としてCRAWLER_LIMIT=2をenvに追加
    # (通しで正しく動作するかどうかの確認のため、各サイトから2件ずつ取得する制限を設けます)
    os.environ["CRAWLER_LIMIT"] = "2"
    ...
    try:
        ...
        run_command([
            ...
            "--parallel=2"
        ], "Step 1/5: Parallel Crawling (All websites, limit 2 per site)")
