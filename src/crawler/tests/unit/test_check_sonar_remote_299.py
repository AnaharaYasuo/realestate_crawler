...
def test_non_dict_response_raises_api_exception(capsys):
    ...
    with patch("urllib.request.urlopen", return_value=mock_resp):
        with pytest.raises(SonarApiException) as exc_info:
            fetch_quality_gate(pr_number=287)
...
def test_invalid_scheme_raises_api_exception():
    """http/https以外の不正なスキームはSonarApiExceptionで拒否されること"""
    from scripts.debug_tools.check_sonar_remote import _execute_api_get
    with pytest.raises(SonarApiException) as exc_info:
