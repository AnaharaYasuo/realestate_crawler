...
class TestTokyuParser:
    ...
    def test_create_entity_investment(self):
        ...
        from package.models.tokyu import TokyuInvestment
        
        parser = TokyuInvestmentParser()
        entity = parser.createEntity()
        assert isinstance(entity, TokyuInvestment)
    ...
    def test_parse_investment_mansion(self):
        if not os.path.exists(os.path.join(os.path.dirname(__file__), '..', 'data', 'tokyu_investment_mansion_mock.html')):
             pytest.skip("tokyu_investment_mansion_mock.html not found")
             return
             
        from package.parser.tokyuParser import TokyuInvestmentParser
        parser = TokyuInvestmentParser()
        item = parser.createEntity()
        html_content = load_mock_html('tokyu_investment_mansion_mock.html')
        soup = BeautifulSoup(html_content, 'html.parser')
