...
**症状:**
`task default` 実行後、コンテナが `Exit` 状態になる

**対処:**
[トラブルシューティングガイド](troubleshooting.md) の「コンテナが起動しない」セクションを参照してください。

### クローラーが実行されない

**症状:**
`task crawl` 実行後、データが保存されない
...
`Taskfile.yml` に独自のタスクを追加できます:

```yaml
tasks:
  # 既存のタスク...
  
  # カスタムタスク: データベースバックアップ
  backup:
    desc: "Backup database"
    cmds:
      - docker compose exec mysql mysqldump -u root -proot realestate_crawler > backup_$(date +%Y%m%d).sql
  
  # カスタムタスク: データベースリストア
  restore:
    desc: "Restore database"
    cmds:
      - docker compose exec -T mysql mysql -u root -proot realestate_crawler < {{.BACKUP_FILE}}
```
...
# リストア
task restore BACKUP_FILE=backup_20240101.sql
```

### 環境変数の設定

`Taskfile.yml` で環境変数を設定できます:
...
Taskコマンドを使用することで、複雑なDocker Composeコマンドを簡潔に実行できます。日常的な操作は以下の3つのコマンドで完結します：

1. `task default` - 起動
2. `task crawl COMPANY=... TYPE=...` - クローラー実行
3. `task stop` - 停止

より詳細な情報は、各ドキュメントを参照してください：
- [README.md](../README.md)
