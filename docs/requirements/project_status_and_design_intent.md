...
単なる「不動産情報のクローリング・データベース保存システム」にとどまらず、**「市場の歪みから眠っている優良な投資・割安物件を自動でスクリーニング・検出するスマート不動産分析パイプライン」**を目指しています。
...
graph TD
    ...
    G --> H[割安度/投資価値算出]
    H -->|投資スコア60以上| I[📢 優良物件アラート通知]
```

不動産情報サイトに散らばる非構造的なデータを集約・構造化し、機械学習モデルと先進的なAI（Gemini API）による分析を組み合わせることで、人間が手作業で行っていた物件比較と妥当な価格（理論価格）の査定を自動化します。
...
2.  **各社サイトの仕様変更への追従**
    *   定期的なクロール実行を通してパースエラー（`error_pages/` に退避されるもの）を監視し、サイト構造の変化に対応する。
3.  **サーバーレス環境 (GCP等) への本番デプロイ**
