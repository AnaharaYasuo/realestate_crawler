...
- Deploy 用 GitHub Actions SA（`github-actions-crawler@...`、`secrets.GCP_SERVICE_ACCOUNT` と同一）に `roles/logging.configWriter` を付与し、New Relic log 用 Pub/Sub トピックには topic 限定で `roles/pubsub.admin` を付与すること。これにより `logging.sinks.create` と sink writer の topic IAM 更新が CI Terraform Apply で成功する（プロジェクト全域の pubsub.admin は付与しない）。
