...
### 3.1 性能・リソース要件
- **バッチ処理時間**: 全サイトの巡回・評価処理（分散並列化により従来の直列実行から大幅に短縮）。
- **コンテナスペック**: 1タスクあたり 2〜4 vCPU、4〜8 GiB RAM を確保可能であること。
- **並列拡張性 (Cloud Run Jobs Task Array)**:
  - 最大20タスク（デフォルト8タスク、同時実行数4タスク）の並列分散タスクアレイを実行し、サイト×種別ジョブを重複なく自動分散できること。
...
- **Coordinator バリア同期**:
  - タスクアレイ環境下で、Task 0 がDBマイグレーションおよび他全タスク完了後の後続処理（データ検証、ML再学習、バルク推論、Slack通知）を一括統括し、重複実行を防止できること。
