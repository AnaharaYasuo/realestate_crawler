...
1. **一括事前検証エンジン (`pre_pr_check.py`)**
   ...
   - **REQ-3 (Linter & 静的解析)**: Ruff、SonarCloud（S3776 <= 15, S8786 ReDoS防止）、Python AST構文検査の実行。
   - **REQ-4 (テストスイート実行)**: 単体テスト（`pytest -n auto src/crawler/tests/unit/`）および変更に応じた関連テストの自動実行。
   - **REQ-5 (PR Mutation Testing)**: `run_mutation_testing.py --pr-mode --threshold=80` の実行と合格判定。
   - **REQ-6 (セキュリティスキャン)**: Semgrep SAST（Python）、Checkov（Terraform変更時）の検証。
   - **REQ-7 (PRメタデータ事前検査)**: PRタイトルフォーマット（`[#<issue_num>] ...`）、本文の `Closes #<issue_num>`、およびPR本文に未完了チェックボックス（`- [ ]`）が存在しないことの事前検証。
2. **実行モード要件**
   - `--full`: 全7ステージ完全実行（PR作成時の必須ゲート）。
   - `--diff`: 変更ファイル対象の高速検証（コミット・プッシュ時向け）。
   - `--fix`: Linter自動修正オプション。
3. **厳守・遮断ガードレール要件**
   - `Taskfile.yml`: `task pr-check`（フル実行）、`task pr-check-fast`（差分実行）、`task pr-create`（全件合格時のみPR作成実行）。
   - `.githooks/pre-push`: Issue受入基準および静的構文チェックを統合し、不備がある場合はリモートプッシュを水際でブロック。
   - `.agents/AGENTS.md` & `git-push-workflow`: PR提出前にローカル100%合格を客観的ログで確認することを義務化。

## 4. 非機能要件 (Non-Functional Requirements)
- **実行速度**: 高速モード（`--diff`）は5秒以内、フルモード（`--full`）は全テスト・ミューテーションを含めても最小限のオーバーヘッドで完了すること。
