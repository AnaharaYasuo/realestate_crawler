...
#### FR-EST-006: 借地権物件における地代取得および負債減価評価要件
- **スクレイピング抽出（全サイト共通）**: 各不動産サイトの詳細スペックテーブル等に記載された地代（月額・年額）表記（例: 「借地期間・地代（月額）：20年 20,000円」等）を検知し、統一フィールド名 `chidai`（月額円）および `chidaiStr`（元表記）として抽出・保存すること。
- **AI抽出（Single Unified Property Extractor）連携**: スクレイピングで未取得の場合でも、1物件1AIリクエストのプロンプトおよびフォールバック処理から `ground_rent_monthly_yen` を抽出し、未取得物件の地代を補完すること。
- **負債減価による適正価値判定**:
  - **投資用物件**: 実額年間地代（`chidai * 12`）をネット営業純利益（NOI）から直接控除し、収益還元価格およびキャッシュフローを地代負債分減額すること。
  - **実需物件（戸建て・マンション・土地）**: 地代支払負担の資本化価値（還元利回り5.0%仮定: 年間地代 ÷ 0.05）を負債として算定し、ML特徴量および理論価格評価に負債減価ディスカウントとして反映すること。

#### FR-CRW-007: クローリングURL識別子維持および0件取得防止要件
- **クエリパラメータ識別子維持 (`UrlMatcher`)**: rearie等、クエリパラメータ（`?id=` 等）で一意の物件詳細を識別するサイトにおいて、URL正規化処理で一意識別用パラメータを保護・維持し、同一URLへの衝突・上書きによる0件取得（Zero-Count Failure）を防止すること。
- **マルチ階層クローリングのセッション自律管理 (`Daikyo`)**: 都道府県・地域別階層構造を展開して詳細物件をクロールするパーサーにおいて、内部HTTPリクエスト用のセッションを自律管理し、外部からセッションが渡されない場合でも一覧・詳細の到達を完結させること。
- **並列上限定数定義の堅牢化 (`Nomura` / `Tokyu`)**: クラウド・ローカル双方の並列リクエスト制御において、未定義定数（`DEFAULT_PARARELL_LIMIT` 等）参照による NameError 実行時例外を根絶すること。
...
#### FR-010: 全国・広域クロールおよび完全ページネーション巡回要件
システムは、各社サイトの物件データを数万件規模で漏れなく収集するため、以下の要件を満たすこと。

1. **広域・全国クロール網羅**:
   - 特定都道府県（東京都のみ等）や特定市区町村（単一区のみ等）への過度な限定指定を廃止し、全国47都道府県または各社が物件を展開する全対象エリアを巡回すること。
   - 大手ポータル（athome, homes等）および全国規模各社（大京, 大和ハウス, 住信SBI, 住友林業, 小田急等）において、全国・広域エリアを起点とした巡回ルートを確立すること。
2. **ページネーションの完全走査**:
   - サイト側のCSS設計変更（Tailwind CSS化、カスタムクラス、アイコン化等）に柔軟に追従し、次ページリンク（`parseNextPage`）を確実に検知して最終ページまで走査を継続すること。
   - 1ページ目のみで巡回が早期終了する不具合を排除し、全ページに渡る完全取得を保証すること。
3. **多段エリア選択の整合ドリルダウン**:
   - エリア選択が階層化（都道府県 ➔ エリア選択 ➔ 市区町村 ➔ 物件一覧）されているサイト（三井, 東急等）において、親ページを物件一覧と誤認することなく、最末端の物件一覧ページまで確実に到達すること。
4. **API認証・アクセス制御ヘッダの適合**:
   - リファラー検証（Refererチェック）等を行うサイト（京王等）に対しては適切なHTTPヘッダを付与し、`403 Forbidden` や取得失敗を防止すること。
...
#### FR-023: クローリング実行状況レポートの時間粒度向上要件 (開始・終了・所要時間内訳)
- 一括巡回バッチ（`run_all_crawlers.py`）完了時のSlack通知および日次JSONレポートにおいて、バッチ全体の「開始時間」「終了時間」「所要時間（〇時間〇分〇秒）」を明記すること。
- 物件件数内訳（会社×物件種別）および異常発生クローラーの各行において、全体サマリーと同等の粒度で「ジョブ開始時間」「ジョブ終了時間」「個別所要時間」を付与して出力すること。
- 単一の物件種別専用サイト（旭化成ストックヘーベル等）において、他種別への同件数誤登録を排除する種別アサーション（ガード機能）を備えること。

#### FR-027: Cloud Run 内部ルーティング保証 & 廃止済旧 URL 排除要件
- Cloud Run Jobs (`IS_CLOUD=true`) 環境において、クローラーの内部ルーティング（地域・一覧・詳細ページ遷移）が正常に行われるよう、`ApiRegistry` を通じた同一プロセス内ローカルディスパッチを常に許可すること。
- 旧アーキテクチャの廃止済み URL（`https://us-central1-sumifu.cloudfunctions.net`）へのハードコードや依存をリポジトリ全体から完全排除し、未設定時のフォールバックをローカルベース URL（`http://127.0.0.1:8000` または `API_BASE_URL` 環境変数）に統一すること。

#### FR-028: タスクアレイ集約レポート要件 (全89ジョブ完全可視化)
- Cloud Run Jobs のタスクアレイ実行（`task_count > 1`）時、各タスクは自身の担当範囲（`Task X/Y 担当: Z件`）を明示して進捗を記録・通知すること。
- Coordinator（Task 0）は、全ワーカータスクの完了を待機後、各タスクの実行結果（`CrawlerTaskExecution.results_json`）を網羅的に集約し、全89ジョブの実行状況（成功件数・失敗件数・未実行件数・個別失敗内訳）が一覧可能な統合集約レポートを Slack およびログへ出力すること。
...
#### NFR-013: GitHub Issue アクセプタンスクライテリアPR制限ゲート要件 (Acceptance Criteria PR Gate)
- **受入基準全件充足の義務化**: Pull Request を提出・マージする際は、紐付けられた GitHub Issue のアクセプタンスクライテリア（受入基準チェックボックス: `- [ ]`）がすべて検証・充足され、チェック済み（`- [x]`）でなければならない。
- **CIマージブロック**: GitHub Actions（`.github/workflows/issue-gate.yml`）により、PRに紐づく Issue を取得し、未チェックの受入基準（`- [ ]`）が1件でも残存している場合は PR の CI を FAIL とし、マージを物理的にブロックすること。
- **受入基準未定義の防止**: Issue本文にチェックボックスが1件も定義されていない場合も、受入基準未策定としてマージ不可とすること。
- **ローカル事前検証 & PR作成ガード**: ローカルCLI（`check_issue_criteria.py`）およびTaskコマンド（`task pr-check`, `task pr-create`）により、受入基準の充足状態をローカルで即座に確認でき、未達時のPR作成を未然に防止すること。

#### NFR-021: 外部API・リモート検査の有限時間タイムアウト保証要件 (External API Finite Timeout)
- **無限待機（ハング）の根絶**: SonarCloud や外部サービスのリモート API 問い合わせ、curl コマンド等を実行する際、タイムアウト未指定による無限待機（ハング）およびバックグラウンドプロセスの実行放置を完全に防止すること。
- **厳格な有限時間タイムアウト**: リモート API コールは必ず有限時間（デフォルト10秒等、最大でも設定された秒数以内）でタイムアウトし、ネットワーク遅延や応答不達時にも安全にプロセスが終了すること。
- **適用範囲**: 本要件は SonarCloud に限定されず、**クローラーを含むすべての外部リモート API 通信・curl コマンド実行**に適用される。各外部 API 呼び出しは有限時間内に完了することを義務付ける（NFR-003 と連携：タイムアウト後もクローラー処理自体は継続する）。
- **専用検証スクリプトの提供**: SonarCloud Quality Gate 状態や未解消課題の確認は専用のタイムアウト保証スクリプト（`check_sonar_remote.py`）を介して実行し、PR番号やブランチ名指定で安全に結果を取得できること。`--timeout` オプションの最大値は 10 秒とし、超過値はスクリプトが CLI 入力エラーとして拒否すること。
