...
#### NFR-010: CIパイプライン並列化・高速化要件
- GitHub Actions CIの実行時間を短縮するため、テストおよびビルド処理を並列化・高速化すること
...
- **実行時間目標**: PR時のテストCI（`Parser Tests`）実行時間を1分台、SonarCloud解析を含む全テストCIを3分前後に短縮すること
