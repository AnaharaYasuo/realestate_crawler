...
CodeRabbit の自動レビュー内にあるタスク項目（`Fix CodeRabbit comments on this PR` 等）や、PR 概要のタスクリストが未チェックのまま残っている場合、マージ不可対象として記録します。なお、無効化済みのレビュー（`state: DISMISSED`）および CodeRabbit の選択肢（`radioGroupId` を含む単一選択・単体テスト生成トリガー等のラジオボタン）はタスクではないため除外判定されます。
