...
| 価格（数値） | `price` | Integer/BigInt | パース後の数値（※必ず円単位。例: 1億円=100000000、4590万円=45900000） |
...
| 地代（文字列） | `chidaiStr` | Text | 全物件種別共通（借地権・定期借地） |
| 地代（数値・月額円） | `chidai` | Integer | 全物件種別共通（借地権・定期借地） |
...
| 2026-09-20 | 地代フィールド（chidaiStr, chidai）を費用関連に追加。全物件種別で借地権負債評価に利用 | - |
