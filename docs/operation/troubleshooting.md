...
システムの詳細な動作を確認するには、コンテナ内のログファイルを参照してください。

- **標準出力**: `docker compose logs -f app`
  - 重要なエラーや保存成功メッセージが表示されます。
- **デバッグログ**: `/app/src/logs/debug.log`
  - クロール中の各URLへのリクエストや、ローカルルーティングの進捗など、詳細な解析情報が含まれます。
  - 確認コマンド: `docker compose exec app tail -f /app/src/logs/debug.log`

## よくあるエラーと対策

### 1. (1264, "Out of range value for column...")
データベースへの保存時に数値がカラムの許容範囲を超えている場合に発生します。
- **原因**: パーサーでの数値変換ミスや、想定外の巨大な値（例：管理費が異常に高い）。
- **対策**: 対象物件のURLを `debug.log` から特定し、パーサーの正規化ロジックを修正してください。

### 2. TypeError: object NoneType can't be used in 'await' expression
`_callApi` メソッドが `async def` で定義されていない場合に発生します。
- **対策**: 対象の API クラスの `_callApi` を `async def` に変更し、引数に `urlList` を追加してください。
